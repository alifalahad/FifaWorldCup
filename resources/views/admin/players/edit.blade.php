...
@section('breadcrumb', 'Admin › Players › ' . $player->first_name . ' ' . $player->last_name)
...
<div class="max-w-2xl">
    ...
    @endif
    <form method="POST" action="{{ route('admin.players.update', $player->player_id) }}" class="bg-white rounded-lg border border-gray-200 p-6">
        @csrf @method('PATCH')
        ...
        <div class="flex items-center justify-between gap-3 mt-6 pt-6 border-t border-gray-100">
            <form method="POST" action="{{ route('admin.players.destroy', $player->player_id) }}"
                ...
                @csrf @method('DELETE')
                <button type="submit" class="text-sm text-red-600 hover:text-red-800 font-medium">Delete this player</button>
            </form>
